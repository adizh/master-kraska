...
const prodDescr = computed(() => {
    return isProdDesctLong ? props?.item?.description?.slice(0, descriptionLen.value) : props?.item?.description
})



const prodSurface = computed(() => {
    return isSurfaceLong ? props?.item?.surfacePreparations?.slice(0, surfaceLen.value) : props?.item?.surfacePreparations
