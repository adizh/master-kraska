...
<div>
    ...
    <input type="text" v-model="newsInputs.nameRu.value" class="basic-input" @input="validate('nameRu','string')">
    <span class="err-input-msg" v-if="newsInputs.nameRu.error">{{ newsInputs.nameRu.error }}</span>
</div>
<div>
    <label for="name">Название (кырг)</label>
    <input type="text" v-model="newsInputs.nameKg.value" class="basic-input" @input="validate('nameKg','string')">
    <span class="err-input-msg" v-if="newsInputs.nameKg.error">{{ newsInputs.nameKg.error }}</span>
</div>


<div>
    <label for="name">Описание</label>
    <input type="text" v-model="newsInputs.descriptionRu.value" class="basic-input" @input="validate('descriptionRu','string')">
    <span class="err-input-msg" v-if="newsInputs.descriptionRu.error">{{ newsInputs.descriptionRu.error }}</span>
</div>

<div>
    <label for="name">Описание (кырг)</label>
    <input type="text" v-model="newsInputs.descriptionKg.value" class="basic-input" @input="validate('descriptionKg','string')">
    <span class="err-input-msg" v-if="newsInputs.descriptionKg.error">{{ newsInputs.descriptionKg.error }}</span>
</div>

<div>
    <label for="name">Текст</label>
    <textarea type="text" v-model="newsInputs.textRu.value" class="basic-input" @input="validate('textRu','string')"></textarea>
    <span class="err-input-msg" v-if="newsInputs.textRu.error">{{ newsInputs.textRu.error }}</span>
</div>

<div>
    <label for="name">Текст (кырг)</label>
    <textarea type="text" v-model="newsInputs.textKg.value" class="basic-input" @input="validate('textKg','string')"></textarea>
    <span class="err-input-msg" v-if="newsInputs.textKg.error">{{ newsInputs.textKg.error }}</span>
</div>

<div>
    <label for="name">Активный</label>
    <input type="checkbox" v-model="newsInputs.isActive.value" class="basic-input"/>
...
<div>
  ...
  >
  <span class="err-input-msg" v-if="newsInputs.image.loading">Загрузка</span>
  <span class="err-input-msg" v-if="!newsInputs.image.loading && newsInputs.image.error">{{ newsInputs.image.error }}</span>
...
const handleNewVarImage = async (event: any) => {
 ...
 newsInputs.value.image.error=''
  ...
  if (result?.size > targetSizeBytes) {
    const bigOne =await useConvertToBase64(event?.target?.files[0]) as unknown as string;
  ...
  const selectNews = (item: News) => {
    ...
    for(const key in newsInputs.value){
        newsInputs.value[key as keyof typeof newsInputs.value].value = item[key as keyof typeof item]
    }
...
const submitEdit =async()=>{
 let body={} as any;
 for(const key in newsInputs.value){
    body[key as keyof typeof body] = newsInputs.value[key as keyof typeof newsInputs.value]?.value
 }
...
setTimeout(()=>{
  ...
  for (const fieldName in newsInputs.value) {
    if (
        fieldName!=='image' && fieldName !== "isActive"  
    ) {
      const fieldType = newsInputs.value[fieldName as keyof typeof newsInputs.value]?.type;
  ...
  if(!hasError && newsInputs.value.image?.value?.length && !newsInputs.value?.image?.error?.length){
