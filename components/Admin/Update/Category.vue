          ...
          <div>
            <ul class="ui-options" v-if="openCategory">
              <input
                ...
                class="basic-input"
                @input="
                  (event: any) => catalogStore.filterLinkedCategories(event?.target?.value)
                "
              />
              <li
...
<div>
    ...
    <input type="text" v-model="categoryInputs.nameRu.value" class="basic-input" @input="validate('nameRu','string')">
    <span class="err-input-msg" v-if="categoryInputs.nameRu.error">{{ categoryInputs.nameRu.error }}</span>
</div>
<div>
    <label for="name">Название (кырг)</label>
    <input type="text" v-model="categoryInputs.nameKg.value" class="basic-input" @input="validate('nameKg','string')">
    <span class="err-input-msg" v-if="categoryInputs.nameKg.error">{{ categoryInputs.nameKg.error }}</span>
</div>

<div>
    <label for="name">URL</label>
    <input type="text" v-model="categoryInputs.url.value" class="basic-input" @input="validate('url','string')">
    <span class="err-input-msg" v-if="categoryInputs.url.error">{{ categoryInputs.url.error }}</span>
</div>

<div>


    <div class="ui-dropdown col-6" v-if="categoryInputs?.parentId?.value!==null">            <label for="name">Категория родителя</label>
        <div class="selected-option basic-input" @click="isTopCategoryOpen=!isTopCategoryOpen">
...
<div>
    ...
    <input type="checkbox" v-model="categoryInputs.isActive.value" class="basic-input"/>
</div>
  ...
  const selectCategory = async(item: CategorySys) => {
    ...
    if(item?.parentId){
      await  catalogStore.fetchLinkedCategoryById(item?.parentId);
        ...
        console.log('catalogStore.getLinkedCategory',catalogStore.getLinkedCategory)
    }
    for (const key in categoryInputs.value) {
  const inputValue = item[key as keyof typeof categoryInputs.value];
  if (inputValue !== null && inputValue !== undefined) {
    categoryInputs.value[key as keyof typeof categoryInputs.value].value = inputValue;
  }
}
...
const submitEdit =async()=>{
 ...
 for(const key in categoryInputs.value){
    if(key!=='parentId'){
        body[key as keyof typeof body] = categoryInputs.value[key as keyof typeof categoryInputs.value]?.value
    }
...
body ={...body,parentId:parentTitle?.value?.id}
  console.log('body',body)
    try{
      const response = await http.put(`/api/v1/Category/update-category/${selectedCategory?.value?.id}`,body)
...
emit('closeModal')
  ...
  for (const fieldName in categoryInputs.value) {
    if (
        fieldName === 'isActive' || fieldName === "parentId"  
    ) {
      const fieldType = categoryInputs.value[fieldName as keyof typeof categoryInputs.value]?.type;
