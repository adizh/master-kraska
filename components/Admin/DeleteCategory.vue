    ...
    <Transition name="slide-fade">
     <div>
      <ul class="ui-options" v-if="openCategory">
          <input type="text" class="basic-input" @input="(event:any)=>catalogStore.filterLinkedCategories(event?.target?.value)"/>
        <li
...
try{
const response = await http.delete(`/api/v1/Category/delete-category-by-id/${category?.id}`);
...
const categoryTitle =computed(()=>{
  if(selectedCategories?.value?.length>0){
    return selectedCategories?.value?.map((item)=>item?.nameRu[0]?.toUpperCase() + item?.nameRu?.slice(1))?.join('. ')
  }else{
...
const index = selectedCategories.value.findIndex((category)=>category?.id===item?.id);
