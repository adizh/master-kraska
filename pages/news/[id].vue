...
if(response.status===200){
    if(authStore?.getSelectedLang==='kg'){
        singleNew.value={...response.data,name:response.data?.nameKg,text:response.data?.textKg,description:response.data?.descriptionKg}
    }else{
        singleNew.value={...response.data,name:response.data?.nameRu,text:response.data?.textRu,description:response.data?.descriptionRu}
    }
}
    }catch(err){
        console.log(err)
