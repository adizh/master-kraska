<template>
    <div class="seller-block">
     ...
     <div v-else>
        ...
        <span class="err-input-msg" v-else-if="!updateImage.loading && updateImage.error">{{
            updateImage?.error
          }}</span>
     </div>
        ...
        <div v-else class="mt-3 mb-5">
            ...
            <input type="file" id="certificateImage" @change="handleCertificateImageUpdate">
            <span v-if="updateCertificateImage.loading">Загрузка...</span>
            <span class="err-input-msg" v-else-if="!updateCertificateImage.loading && updateCertificateImage.error">{{
                updateCertificateImage?.error
              }}</span>
         </div>
      </div>
    ...
    <div v-else class="mb-4">
      ...
      <div v-else class="mb-4">
        ...
        <textarea class="basic-input"  v-model="localSeller.descriptionKg"></textarea>
      </div>
      <p v-if="!isEdit">{{ seller?.descriptionRu }}</p>
      <div v-else class="mb-4">
        <label for="name">Описание</label>
        <textarea class="basic-input"  v-model="localSeller.descriptionRu"></textarea>
      ...
      <button v-if="isEdit" class="bg-white-btn" type="button" @click="editSeller">
        Обновить
      </button>
    </div>
  </template>
...
import {handleCertificateImageUpdate,handleImageUpdate,updateImage,updateCertificateImage} from '@/helpers/admin/sellers'
...
try{
    const response = await http.put(`/api/v1/Seller/update-seller/${props?.seller.id}`,body)
...
if(localSeller.value.name?.length && localSeller.value.descriptionRu?.length&& localSeller.value.descriptionKg?.length
&& updateImage.value.value && !updateImage.value.error?.length&& updateCertificateImage.value.value && !updateCertificateImage.value.error?.length
    ...
    image: updateImage.value.value?.includes('https') ? null :updateImage.value.value,
    imageExtension: "png",
    certificateImage: updateCertificateImage.value.value?.includes('https') ? null : updateCertificateImage.value.value,
