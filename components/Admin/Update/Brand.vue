  ...
  >
    ...
    <form v-if="isFormOpen" class="admin-edit-form" @submit.prevent="editBrand">
      <div>
        ...
        <input
          type="text"
          v-model="brandsInputs.name.value"
          class="basic-input"
          @input="validate('name', 'string')"
        />
        <span class="err-input-msg" v-if="brandsInputs.name.error">{{
          brandsInputs.name.error
        }}</span>
      </div>
      <div>
        ...
        <input
          type="number"
          v-model="brandsInputs.order.value"
          class="basic-input"
          @input="validate('order', 'number')"
        />
        <span class="err-input-msg" v-if="brandsInputs.order.error">{{
          brandsInputs.order.error
        }}</span>
      </div>
      <div>
      ...
      <div>
        ...
        <span
          class="err-input-msg"
          v-if="!brandsInputs.logo.loading && brandsInputs.logo.error"
          >{{ brandsInputs.logo.error }}</span
        >
        <span class="err-input-msg" v-else-if="brandsInputs.logo.loading"
          >Загрузка</span
        >
      </div>
...
const submitEdit = async () => {
  ...
  if (body.logo?.startsWith("http")) {
    body = { ...body, logo: null,      extension:'png' };
  } else {
  ...
  try {
    const response = await http.put(
      `/api/v1/Brand/update-brand/${selectedBrands?.value?.id}`,
      body,
    );
...
const editBrand = () => {
  for (const fieldName in brandsInputs.value) {
    if (fieldName === "name" || fieldName === "order") {
      const fieldType =
        brandsInputs.value[fieldName as keyof typeof brandsInputs.value]?.type;
