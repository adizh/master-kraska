<template>
  <form class="order-form" @submit.prevent="sendCheckPayment">
    ...
    <div class="flex flex-column gap-2 mt-4">
      ...
      <InputMask
        ...
        v-model="payStore.getMbank.mbankPhone.value"
        @input="payStore.clearError('mbankPhone')"
      />
      <span v-if="payStore.getMbank.mbankPhone.error" class="err-input-msg">{{
        payStore.getMbank.mbankPhone.error
      }}</span>
    </div>
    <div class="btn">
  ...
  <div v-if="payStore.getMbank?.openConfirm">
    <InputOtp
      v-model="payStore.getMbank.otp.value"
      :length="4"
      id="code"
      @input="payStore.clearError('otp')"
    />
    <span v-if="payStore.getMbank.otp.error" class="err-input-msg">{{
      payStore.getMbank.otp.error
    }}</span>
    <div class="btn">
      <button class="bg-white-btn" type="submit" @click="sendOtp">
        {{ $t("submitData") }}
      </button>
    </div>
  </div>
...
const sendCheckPayment = () => {
  console.log(
    ...
    payStore.getMbank.mbankPhone.value,
  );
