  ...
  <form class="order-form" @submit.prevent="sendCheckPayment">
    ...
    <div class="flex flex-column gap-2 mt-4">
      ...
      <InputMask
        id="basic"
        mask="999999999999"
        placeholder="996700555555"
        v-model="payStore.getMega.megaPhone.value"
        @input="payStore.clearErrorMega('megaPhone')"
      />
      <span v-if="payStore.getMega.megaPhone.error" class="err-input-msg">{{
        payStore.getMega.megaPhone.error
      }}</span>
    </div>
    <div class="btn">
      <button class="bg-white-btn" type="submit">{{ $t("submitData") }}</button>
    </div>
  </form>
  ...
  <div v-if="payStore.getMega.openConfirm">
    <div class="flex flex-column gap-2">
      ...
      <InputMask
        ...
        mask="999999999999"
        placeholder="996700555555"
        v-model="payStore.getMega.user.value"
        @input="payStore.clearErrorMega('user')"
      />
    </div>

    <span v-if="payStore.getMega.user.error" class="err-input-msg">{{
      payStore.getMega.user.error
    }}</span>

    <div class="flex flex-column mt-3">
      <InputOtp
        v-model="payStore.getMega.otp.value"
        :length="4"
        @input="payStore.clearErrorMega('otp')"
        class="mb-2"
      />
      <span v-if="payStore.getMega.otp.error" class="err-input-msg">{{
        payStore.getMega.otp.error
      }}</span>
    </div>
    <div class="btn">
      <button class="bg-white-btn" type="submit" @click="sendPayment">
        {{ $t("submitData") }}
      </button>
    </div>
  </div>
...
const sendCheckPayment = () => {
  payStore.checkPayment(
    ...
    cartStore.getCurrentOrder?.orderNumber,
    "megapay",
  );
};
