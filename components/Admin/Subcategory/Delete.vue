<template>
    ...
    >
      ...
      >
        ...
        <button type="submit" class="pink-button" :disabled="disabledForm" :class="{'diabled-btn':disabledForm}">Удалить</button>
      </form>
    </UIModal>
  </template>
  ...
  const disabledForm =computed(()=>{
    if(catalogStore.getFilteredSubcategories?.length>0 && selectedSubcategory?.value?.id){
      return false
    }else{
      return true
    }
  ...
  if(selectedSubcategory?.value?.id && selectedCategoryId?.value?.id){
    ...
    try{
      const response = await http.delete(`/api/v1/Helpers/delete-subdirectory-by-id/${selectedSubcategory?.value?.id}`)
